...
export default function EngineLab() {
  ...
  const gestures = useEngineGestures({ mode, scene, onSelect: setSelected, onPart: applyPart, onPause: () => setPlaying(false), onRange: (name, value) => { if (name === "explode") { setExploded(value); if (value > 0) setPlaying(false); } if (name === "cycle") { setPlaying(false); setAngle(value * 719.9); } } });
  ...
  const chooseMode = (next: ViewMode) => {
    ...
    if (next === "run") { setCutaway(true); setXray(false); setAngle(0); }
  };
    ...
    <main className="lab-main">
      ...
      <section className="workbench" aria-label="Engine workbench">
        ...
        <div className="stage-column" ref={stage}>
          ...
          <div className="scene-area">
            <EngineScene ref={scene} selected={selected} removed={removed} mode={mode} exploded={exploded} cutaway={cutaway} xray={xray} isolated={isolated} labels={labels} playing={playing} speed={speed} angle={angle} onSelect={selectPart} onAngle={setAngle}/>
            ...
            <div className="scene-tools"><button className="icon-button" title="Zoom in" aria-label="Zoom in" onClick={() => scene.current?.zoom(.18)}><LabIcon name="zoomIn" size={18}/></button><button className="icon-button" title="Zoom out" aria-label="Zoom out" onClick={() => scene.current?.zoom(-.18)}><LabIcon name="zoomOut" size={18}/></button><span/><button className="icon-button" title="Focus selected part" aria-label="Focus selected part" onClick={() => scene.current?.focus(selected)}><LabIcon name="crosshair" size={18}/></button></div>
          ...
          <div className="stage-footer"><span><LabIcon name="pointer" size={13}/> Drag to orbit <i/> Scroll to zoom <i/> Double-click to focus</span><span className="stage-coordinate">{camera.status === "ready" ? <><span className="status-dot"/> HANDS CONNECTED</> : <><LabIcon name="cube" size={12}/> FREE ORBIT</>}</span></div>
        </div>
      ...
      <section data-camera-status={camera.status} className={`hand-panel ${cameraActive || camera.status === "error" || handHelp ? "expanded" : ""}`} aria-label="Hand controls">
        ...
        <div className="hand-expanded-content">
          ...
          </div>{camera.detail && <p className="camera-detail" role="status">{camera.detail}{camera.cameraName ? ` · ${camera.cameraName}` : ""}</p>}<div className="gesture-title"><strong>{camera.status === "ready" ? gestures.hint : "Your hands are the controller."}</strong>{gestures.paused && camera.status === "ready" && <button onClick={gestures.resume}>Resume controls</button>}</div><div className="gesture-grid"><div><span>01</span><strong>Pinch & move</strong><p>In Inspect, pinch over the engine and move to rotate. Pinch a button to click.</p></div><div><span>02</span><strong>Two hands to zoom</strong><p>Pinch with both hands. Spread apart to zoom in, bring together to zoom out.</p></div><div><span>03</span><strong>Grab an assembly</strong><p>In Assemble, pinch a part, move your hand, then release to remove or install.</p></div><div><span>04</span><strong>Open palm to pause</strong><p>Hold for one second. Hold again to resume. Lost tracking cancels a grab.</p></div></div><div className="camera-privacy"><LabIcon name="lock" size={13}/>Camera frames are processed on this device. No audio is captured.<span>Point at top / bottom screen edge to scroll</span></div>{camera.error && <p className="camera-error" role="alert">{camera.error}</p>}{camera.status === "preview" && <button className="secondary-button" onClick={camera.retryTracking}><LabIcon name="hand" size={15}/>Retry hand tracking</button>}{!cameraActive && <button className="secondary-button" onClick={() => { gestures.resume(); void camera.start(); }}><LabIcon name="camera" size={15}/>{camera.status === "error" ? "Try camera again" : "Enable camera"}</button>}</div>
        </div>
      </section>
